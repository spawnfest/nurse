defmodule NurseWeb.NewCheckLive do
    ...
    def handle_event(
        ...
        new_map = Nurse.CondMap.put_new_son_of(socket.assigns.condition_map, parent_condition_id, "simple", condition_side)
        socket =
    ...
    def handle_event(
        ...
        new_map = 
            Nurse.CondMap.update_condition_params(socket.assigns.condition_map, condition_id, %{status_code_match: status_code_match})
        socket =
            assign(socket, condition_map: new_map)   
        {:noreply, socket}
    end
    ...
    def handle_event(
        ...
        new_map = 
            Nurse.CondMap.update_condition_params(socket.assigns.condition_map, condition_id, %{headers_match: headers_match})
        socket =
            assign(socket, condition_map: new_map)   
        {:noreply, socket}
    end
    ...
    def handle_event(
        ...
        new_map = 
            Nurse.CondMap.update_condition_params(socket.assigns.condition_map, condition_id, %{body_match: body_match})
        socket =
            assign(socket, condition_map: new_map)   
        {:noreply, socket}
    end
        ...
        case Nurse.CondMap.validate(socket.assigns.condition_map) do
            true -> 
                ...
                case inputs do
                    {:field, fieldname} ->
                        socket =
                            assign(socket, bad_input_msg: "Field "<>fieldname<>" is invalid")
                        {:noreply, socket}
                    true ->
            ...
            false ->
                socket =
                    assign(socket, bad_input_msg: "Condition tree needs conditions in all its branches, and can not be empty.")
                {:noreply, socket}
        end
    end
    ...
    defp validate_input_values(
        ...
        "retry_delay" => retry_delay} = properties) 
    do
        properties 
        |> Map.to_list()
        |> validate_multi()
